...
class WormManager extends Effect {
    ...
    constructor(segments, revert=false, fill=false) {
        super(segments)
        ...
        if(this.revert) {
            let temp_unreverted_segs = this.unreverted_segs
            this.unreverted_segs = this.reverted_segs
            this.reverted_segs = temp_unreverted_segs
        }
    }
    ...
    run() {
        ...
        let strob_fc = () => {
            ...
            this.unreverted_segs.forEach(s => {
                if(this.fill) {
                    s.off_between(current_start_at, end_worm_at)
                } else {
            ...
            this.reverted_segs.forEach(s => {
                if(this.fill) {
                    s.off_between(start_worm_at, current_end_at)
                } else {

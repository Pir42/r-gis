...
class Segment {
    ...
    decrease_brightness_by_color(speed=100, bright_decrease_ratio=0.7, callback=()=>{}) {
    ...
    shift_leds(revert=false) {
        if(revert) {
            ...
            for (let i = this.seg_end; i >= this.seg_start; i--) {
                if(this.silent) {
                    this.leds[i] = 0x000000
                } else {
                    this.leds[i] = this.leds[i-1]
        ...
        if(revert) {
            this.leds[this.seg_end] = this.shift_color
        } else {
            this.leds[this.seg_start] = this.shift_color
        }
